...
export default function Tweet({ data, id }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(doc(db, "posts", id), (doc) => {
      setLikes(doc.data()?.likes), setComments(doc.data()?.comments);
    });
    return unsubscribe;
  }, []);
      ...
      <div className="p-3 ml-16 text-gray-500 flex space-x-14">
        ...
        >
          {likes.includes(user.uid) ? (
            <Heart className="w-5 text-pink-500" />
          ) : (
        ...
        {user.uid === data?.uid && (
          <div className="hover:text-red-600" onClick={deleteTweet}>
            <TrashIcon className="w-5" />
          </div>
        )}
        <ChartBarIcon className="w-5 cursor-not-allowed " />
        <UploadIcon className="w-5 cursor-not-allowed hover:text-green-500" />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex space-x-3 p-3">
      ...
      <div>
        ...
        {image && (
          <img
            className="object-cover border border-gray-700 rounded-md mt-3 max-height-80"
            src={image}
          />
        )}
      </div>
